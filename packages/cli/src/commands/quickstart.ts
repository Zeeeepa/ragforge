...
export interface QuickstartOptions {
  ...
  replicateToken?: string; // Replicate API token for 3D/image generation
  ...
  if (options.dev) {
    ...
    try {
      ...
      try {
        ...
        if (!replicateToken) {
          const replicateMatch = envContent.match(/^REPLICATE_API_TOKEN=(.+)$/m);
          if (replicateMatch) {
            replicateToken = replicateMatch[1].trim();
            console.log('✓ Using REPLICATE_API_TOKEN from monorepo .env (dev mode)');
          }
        }
      } catch {
...
export async function runQuickstart(options: QuickstartOptions): Promise<void> {
  ...
  if (needsNeo4jCredentials && !containerExists) {
    ...
    if (existingEnvContent) {
      ...
      // If we found a geminiKey (e.g., from monorepo .env in dev mode), use it
      if (options.geminiKey) {
        newEnvContent += `GEMINI_API_KEY=${options.geminiKey}\n`;
        console.log('✓ GEMINI_API_KEY copied to project .env');
      } else {
        newEnvContent += '# GEMINI_API_KEY=your-api-key-here\n';
      }
      // Replicate token for 3D/image generation
      if (options.replicateToken) {
        newEnvContent += `REPLICATE_API_TOKEN=${options.replicateToken}\n`;
        console.log('✓ REPLICATE_API_TOKEN copied to project .env');
      } else {
        newEnvContent += '# REPLICATE_API_TOKEN=your-token-here\n';
      }
      newEnvContent += '\n# Neo4j Configuration (auto-generated by quickstart)\n';
    }
  ...
  if (options.ingest && mergedConfig.source) {
    ...
    // Universal ingestion (handles all file types: code, documents, media, data)
...
/**
 * Parse and ingest source files into Neo4j
 * Uses UniversalSourceAdapter which handles all file types:
 * - Code: .ts, .tsx, .js, .jsx, .py, .vue, .svelte, etc.
 * - Documents: .pdf, .docx, .xlsx, .csv
 * - Data: .json, .yaml, .md
 * - Media: .png, .jpg, .glb, .gltf
 */
async function parseAndIngestSource(
  ...
  console.log('\n📦 Parsing and ingesting source files...');
  console.log(`📁 Root: ${sourceConfig.root || process.cwd()}`);
  console.log(`📝 Adapter: universal (auto-detect by extension)`);

  // Create universal adapter (handles all file types)
  const adapter = new UniversalSourceAdapter();
  ...
  try {
    ...
    // Dynamic schema creation - automatically handles any node type
    console.log(`📋 Creating indexes and constraints...`);

    const BATCH_SIZE = 500;
    const nodesByLabel = groupNodesByLabel(graph.nodes);

    // Create constraints dynamically for each label
    for (const [label, nodes] of nodesByLabel) {
      if (nodes.length === 0) continue;

      const uniqueField = inferUniqueField(nodes[0]);
      const constraintName = getConstraintName(label, uniqueField);

      await client.run(
        `CREATE CONSTRAINT ${constraintName} IF NOT EXISTS FOR (n:${label}) REQUIRE n.${uniqueField} IS UNIQUE`
      );

      // Create additional indexes for this label type
      const additionalIndexes = getAdditionalIndexes(label);
      for (const field of additionalIndexes) {
        await client.run(
          `CREATE INDEX ${label.toLowerCase()}_${field} IF NOT EXISTS FOR (n:${label}) ON (n.${field})`
        );
      }
    }
    console.log(`✓ Schema created`);

    // Create nodes dynamically for each label type
    ...
    for (const [label, nodes] of nodesByLabel) {
      ...
      const uniqueField = inferUniqueField(nodes[0]);
      const cypherValue = getCypherUniqueValue(uniqueField);
      ...
      for (let i = 0; i < nodes.length; i += BATCH_SIZE) {
        ...
        await client.run(
          `UNWIND $nodes AS nodeData
           MERGE (n:${label} {${uniqueField}: ${cypherValue}})
           SET n += nodeData.props`,
          { nodes: batchData }
        );
      }

      // Show progress for larger node sets
      if (nodes.length > 10) {
    ...
    // Group relationships by type and node labels
    const relsByKey = new Map<string, typeof graph.relationships>();
    for (const rel of graph.relationships) {
      const fromLabel = nodeLabelById.get(rel.from) || 'Unknown';
      const toLabel = nodeLabelById.get(rel.to) || 'Unknown';
      const key = `${rel.type}:${fromLabel}:${toLabel}`;
      if (!relsByKey.has(key)) {
        relsByKey.set(key, []);
      }
      relsByKey.get(key)!.push(rel);
    }

    // Batch create relationships
    let totalRelsCreated = 0;
    for (const [key, rels] of relsByKey) {
      const [relType, fromLabel, toLabel] = key.split(':');

      // Get type info for from/to nodes
      const sampleFromId = rels[0].from;
      const sampleToId = rels[0].to;
      const fromInfo = getNodeTypeFromId(sampleFromId);
      const toInfo = getNodeTypeFromId(sampleToId);

      for (let i = 0; i < rels.length; i += BATCH_SIZE) {
        const batch = rels.slice(i, i + BATCH_SIZE);
        const batchData = batch.map(rel => ({
          from: getNodeTypeFromId(rel.from).matchValue,
          to: getNodeTypeFromId(rel.to).matchValue,
          props: rel.properties || {}
        }));

        await client.run(
          `UNWIND $batch AS rel
           MATCH (a:${fromLabel} {${fromInfo.uniqueField}: rel.from})
           MATCH (b:${toLabel} {${toInfo.uniqueField}: rel.to})
           MERGE (a)-[r:${relType}]->(b)
           SET r += rel.props`,
          { batch: batchData }
        );
